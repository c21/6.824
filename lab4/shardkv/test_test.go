...
func TestJoinLeave(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		//DPrintf("before check ka[%d]:%s, va:%s, shard:%d\n",
		//  i, ka[i], va[i], key2shard(ka[i]))
		check(t, ck, ka[i], va[i])
		//DPrintf("after ka[%d]:%s, va:%s\n", i, ka[i], va[i])
		x := randstring(5)
		ck.Append(ka[i], x)
		va[i] += x
	}
	...
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		x := randstring(5)
		//    DPrintf("before append ka[%d]:%s, va:%s, x:%s, shard:%d\n",
		//      i, ka[i], va[i], x, key2shard(ka[i]))
		ck.Append(ka[i], x)
		//    DPrintf("after append ka[%d]:%s, va:%s\n", i, ka[i], va[i])
		va[i] += x
	}
