...
const (
	...
	MIN_ELECTION_TIMEOUT_MS = 150 //2 * HEARTBEAT_TIMEOUT_MS
	MAX_ELECTION_TIMEOUT_MS = 300	//5 * HEARTBEAT_TIMEOUT_MS
)
...
/*
          		...
          		}(rf.lastApplied, rf.log[rf.lastApplied].Command)
*/					
							idx := rf.lastApplied
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if rf.state == LEADER {
		...
		// Send AppendEntries note to peers.
		go func() {
			for i := 0; i < len(rf.peers); i++ {
				...
				if len(rf.log) - 1 >= rf.nextIndex[i] {
					...
					go func() {
						...
						for nextIdx >= 1 {
							rf.mu.Lock()
							DPrintf("[Start server(%d) term(%d)]: command:%d, idx:%d server:%d\n",
								rf.me, currentTerm, command, nextIdx, server)
							args := AppendEntriesArgs{currentTerm, rf.me, nextIdx-1,
              	rf.log[nextIdx-1].Term, rf.log[nextIdx:lastIdx], commitIdx}
							rf.mu.Unlock() 
							if rf.sendAppendEntries(server, args, &reply) && reply.Success {
								rf.mu.Lock()
								if reply.Term > rf.currentTerm {
									rf.mu.Unlock()
									return
                	...
                	rf.me, currentTerm, command, nextIdx, server, rf.nextIndex[server])
								count := 0
								minIdx := 0
