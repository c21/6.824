...
func doMap(
	...
	reduceEncs := make([]*json.Encoder, nReduce) 
	for i := 0; i < nReduce; i++ {
		...
		reduceFiles[i] = file
		reduceEncs[i] = json.NewEncoder(file)		
	}

	// Write key/value pairs to intermediate files.
	for _, kv := range kvs {
		bin := ihash(kv.Key) % uint32(nReduce)
		err := reduceEncs[bin].Encode(&kv)
		if err != nil {
